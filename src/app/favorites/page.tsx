      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8 pb-[calc(88px+env(safe-area-inset-bottom))] lg:pb-8">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm" style={{ color: VIOLET.textSecondary }}>
                Your saved ideas — tap \u2665 on any idea in <Link href="/ideas" className="text-violet-300 underline">Ideas</Link> to add it here. Persists in <span className="font-mono text-white/60">localStorage(fleet_favorites)</span> (Android: <span className="font-mono text-white/60">DataStore</span>). {list.length} saved \u00b7 {newCount} New \u00b7 {enhCount} Enhance.
              </p>
        ...
        {list.length===0 ? (
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-10 text-center">
            ...
            <p className="mt-1 text-sm text-white/50">Go to <Link href="/ideas" className="text-violet-300 underline">Ideas</Link> and tap \u2661 on any card — it turns \u2665 and appears here. Survives reload and restart.</p>
            ...
            {list.map((idea)=>(
              <div key={idea.id} className="flex flex-col rounded-2xl border p-4 sm:p-5 hover:shadow-lg hover:shadow-violet-500/10 transition" style={{ background: VIOLET.surface, borderColor: VIOLET.border }}>
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  <span className="ml-auto text-[11px] text-white/30">\u2605 saved</span>
                </div>
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">{idea.widgets.slice(0,4).map((w)=>(<span key={w} className="rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[11px] font-medium text-white/60">{w}</span>))}</div>
                <div className="mt-2 flex items-center gap-2 text-[11px]"><span className="rounded-full bg-white px-2 py-0.5 font-bold text-[#0f0b1a]">Gap {idea.gapScore}%</span>{idea.targetSlug?<span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-white/60">\u2192 {idea.targetSlug}</span>:null}</div>
                <button onClick={()=>setExpanded(expanded===idea.id?null:idea.id)} className="mt-3 flex w-full items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-left hover:bg-white/[0.06]">
                  ...
                  <span className="text-[11px] text-white/40">Problem \u00b7 Solution \u00b7 Benefit</span>
                </button>
                {expanded===idea.id ? (
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  ...
                  <button onClick={()=>setConfirmIdea(idea)} disabled={scaffolding===idea.id} className={`inline-flex min-h-[40px] items-center justify-center rounded-full px-2 text-[12px] font-semibold disabled:opacity-50 ${idea.kind==="enhancement"?"border border-amber-500/30 bg-amber-500/15 text-amber-200 hover:bg-amber-500/20":"bg-violet-600 text-white hover:bg-violet-500"}`}>{scaffolding===idea.id?"\u2026":idea.kind==="enhancement"?"Scaffold tab \u2192 "+(idea.targetSlug||""):"Create dashboard"}</button>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-2">
                  ...
                  <button onClick={()=>copyImprove(idea)} className="inline-flex min-h-[36px] items-center justify-center rounded-full border border-amber-500/20 bg-amber-500/10 px-2 text-[11px] font-semibold text-amber-200 hover:bg-amber-500/15">Copy IMPROVE</button>
                </div>
              </div>
            ))}
          </div>
