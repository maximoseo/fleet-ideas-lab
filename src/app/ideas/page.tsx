      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8 pb-[calc(88px+env(safe-area-inset-bottom))] lg:pb-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-sm" style={{ color: VIOLET.textSecondary }}>{FLEET_IDEAS.length} professional briefs — deduplicated against {FLEET_COUNT} live dashboards. <span className="font-semibold text-emerald-200">5 New dashboards</span> (white-space) + <span className="font-semibold text-amber-200">6 Enhancements</span> (add as tab inside existing dashboard) — 1 duplicate removed (Content Decay already live). Tap to expand full brief with evidence.</p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex gap-1 rounded-full border border-white/10 bg-white/[0.04] p-1" role="tablist" aria-label="Ideas view">
              {(["list", "board"] as const).map((v) => (
                <button key={v} role="tab" aria-selected={view === v} onClick={() => setView(v)} className={`min-h-[32px] rounded-full px-4 text-[12px] font-semibold capitalize transition ${view === v ? "bg-violet-600 text-white" : "text-white/75 hover:text-white"}`}>{v === "list" ? "List" : "Board"}</button>
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className="flex flex-wrap gap-1.5 rounded-full border border-white/10 bg-white/[0.04] p-1">
              {(["all", "seo", "content", "local", "analytics", "automation", "design", "outreach", "technical"] as const).map((d) => (
                <button key={d} onClick={() => setDomain(d as FleetDomain | "all")} className={`min-h-[32px] rounded-full px-3 text-xs font-semibold transition ${domain === d ? "bg-violet-600 text-white" : "text-white/75 hover:text-white"}`}>{d === "all" ? "All Domains" : DOMAIN_LABEL[d]}</button>
              ))}
            </div>
            <div className="flex gap-1 rounded-full border border-white/10 bg-white/[0.04] p-1">
              {(["all", "new", "enhancement"] as const).map((k) => (
                <button key={k} onClick={() => setKind(k)} className={`min-h-[32px] rounded-full px-3 text-xs font-semibold capitalize ${kind === k ? "bg-violet-600 text-white" : "text-white/75 hover:text-white"}`}>{k === "all" ? "All kinds" : k === "new" ? "New" : "Enhancement"}</button>
              ))}
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex gap-1 rounded-full border border-white/10 bg-white/[0.04] p-1">
              {(["all", "S", "M", "L", "XL"] as const).map((e) => (
                <button key={e} onClick={() => setEffort(e as unknown as Effort)} className={`min-h-[32px] rounded-full px-3 text-xs font-semibold ${effort === e ? "bg-white text-[#0f0b1a]" : "text-white/75 hover:text-white"}`}>{e === "all" ? "Any Effort" : e}</button>
              ))}
            </div>
            <div className="flex gap-1 rounded-full border border-white/10 bg-white/[0.04] p-1">
              {(["all", "high", "medium", "low"] as const).map((v) => (
                <button key={v} onClick={() => setImpact(v as unknown as Impact)} className={`min-h-[32px] rounded-full px-3 text-xs font-semibold capitalize ${impact === v ? "bg-white text-[#0f0b1a]" : "text-white/75 hover:text-white"}`}>{v === "all" ? "Any Impact" : v}</button>
              ))}
            </div>
            <div className="flex gap-1 rounded-full border border-white/10 bg-white/[0.04] p-1">
              {(["all", "new", "scoped", "backlog", "shipped"] as const).map((v) => (
                <button key={v} onClick={() => setStatus(v as unknown as IdeaStatus)} className={`min-h-[32px] rounded-full px-3 text-xs font-semibold capitalize ${status === v ? "bg-white text-[#0f0b1a]" : "text-white/75 hover:text-white"}`}>{v}</button>
              ))}
            </div>
            <div className="ml-auto flex items-center gap-2 w-full sm:w-auto">
              <input ref={searchRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search ideas, problem, solution…" className="w-full sm:w-64 rounded-full border border-white/10 bg-white/[0.06] px-4 py-2.5 text-sm text-white placeholder:text-white/60 focus:border-violet-500 focus:outline-none" />
              <span className="hidden sm:inline text-xs text-white/65">{filtered.length} shown · {ALL_POOL.length - unseenLeft} seen · {unseenLeft} unseen left</span>
            </div>
          </div>
        </div>
        </>
        ...
        <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-[11px] leading-4 text-white/65">
          On <span className="font-mono text-white/75">fleet-ideas-lab.vercel.app</span> scaffolds land in <span className="font-mono text-violet-200">/tmp/&lt;slug&gt;</span> (ephemeral — Vercel sandbox). On Hostinger <span className="font-mono text-white/75">srv1813877</span> they persist at <span className="font-mono text-violet-200">/root/projects/&lt;slug&gt;</span>. History keeps a trace either way — see <a href="/history" className="text-violet-200 underline">History</a>.
        </div>
          ...
          {filtered.map((idea) => (
            <div key={idea.id} className="flex flex-col rounded-2xl border p-4 sm:p-4 xl:p-3 hover:shadow-lg hover:shadow-violet-500/10 transition" style={{ background: VIOLET.surface, borderColor: VIOLET.border }}>
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                <button onClick={() => toggleFav(idea.slug)} className={`ml-1 rounded-full px-2 py-0.5 text-[12px] font-bold border ${favs.has(idea.slug) ? "bg-amber-500 text-black border-amber-500" : "bg-white/5 text-white/65 border-white/10 hover:text-white"}`}>{favs.has(idea.slug) ? "\u2665" : "\u2661"}</button>
                ...
                {idea.id.startsWith("idea-research") ? <span className="rounded-full bg-sky-500/15 border border-sky-500/30 px-2 py-0.5 text-[10px] font-bold text-sky-100">Fresh from web \u00b7 2026-08-16</span> : null}
                <span className={`ml-auto rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize ${idea.impact === "high" ? "bg-emerald-500/15 text-emerald-200" : idea.impact === "medium" ? "bg-amber-500/15 text-amber-200" : "bg-white/10 text-white/75"}`}>{idea.impact}</span>
              </div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2 text-[11px]">
                ...
                {idea.targetSlug ? <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-white/75">→ {idea.targetSlug}</span> : null}
                <span className="text-white/60">{idea.kind === "new" ? "New dashboard" : "Add as tab inside existing"}</span>
              </div>
              <div className="mt-2 rounded-lg border border-amber-500/15 bg-amber-500/5 px-3 py-2 text-[11px] leading-4 text-amber-50/90">
                <span className="font-bold text-amber-100">Evidence (inline):</span> {idea.evidence}
              </div>
              ...
              <button onClick={() => setExpanded(expanded === idea.id ? null : idea.id)} className="mt-3 flex w-full items-center justify-between rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-left hover:bg-white/[0.06]">
                ...
                <span className="text-[11px] text-white/65">Problem \u00b7 Solution \u00b7 Benefit \u00b7 Data \u00b7 Feasibility</span>
              </button>
              ...
              {expanded === idea.id ? (
                <div className="mt-3 space-y-3 rounded-xl border border-violet-500/20 bg-violet-500/[0.06] p-4 text-[12px] leading-5">
                  ...
                  <div><span className="font-bold text-white/75">Feasibility:</span> <span className="text-white/75">{idea.feasibility}</span></div>
                  <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3"><span className="font-bold text-amber-100">Evidence (vs {FLEET_COUNT} live):</span> <span className="text-amber-100/80">{idea.evidence}</span></div>
                  <div className="rounded-lg bg-white/5 p-3"><span className="font-bold text-white">Next step:</span> <span className="text-violet-200">{idea.nextStep}</span> {idea.targetSlug ? <span className="text-white/65">· target: {idea.targetSlug}</span> : null}</div>
                </div>
              ) : null}
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <button onClick={() => openNotifyPicker(idea)} disabled={notifying === idea.id} className="inline-flex min-h-[36px] items-center justify-center rounded-full border border-sky-500/30 bg-sky-500/15 px-2 text-[11px] font-bold text-sky-100 hover:bg-sky-500/25 disabled:opacity-50">{notifying === idea.id ? "… Sending" : "📨 Send to Bot"}</button>
              ...
              <div className="mt-2 grid grid-cols-3 gap-2">
                <button onClick={() => copyPrompt(idea)} title={resolveMode(idea) === "improve" ? "Copy IMPROVE brief (Markdown) — optimize existing " + (idea.targetSlug || idea.slug) : "Copy BUILD brief (Markdown) — new dashboard " + idea.slug} className={`inline-flex min-h-[36px] items-center justify-center rounded-full border px-2 text-[11px] font-bold ${resolveMode(idea)==="improve" ? "border-amber-500/30 bg-amber-500/15 text-amber-100 hover:bg-amber-500/20" : "border-violet-500/30 bg-violet-600 text-white hover:bg-violet-500"}`}>{resolveMode(idea)==="improve" ? "Copy IMPROVE" : "Copy BUILD"} · Auto</button>
        ...
        <div ref={sentinelRef} className="flex flex-col items-center gap-3 py-6">
          ...
          {endOfFeed && filtered.length>0 ? <span className="text-xs text-white/60">You\u2019ve seen all {filtered.length} for this filter — <button onClick={() => { setDomain("all"); setEffort("all" as unknown as Effort); setImpact("all" as unknown as Impact); setStatus("all" as unknown as IdeaStatus); setKind("all"); setFavOnly(false); setQ(""); }} className="underline text-violet-200">Clear filters</button> or change domain</span> : null}
          {!loadingMore && !endOfFeed && filtered.length>0 && unseenLeft===0 ? <span className="text-xs text-white/60">All {ALL_POOL.length} ideas are already shown — pull to reshuffle or Clear filters</span> : null}
        </div>

        {filtered.length === 0 ? (
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.03] p-10 text-center">
            <p className="text-sm text-white/75">{favOnly && favs.size === 0 ? "\u2606 No favorites yet \u2014 tap \u2661 on any idea" : favOnly ? "No favorites match your filters" : "No ideas match your filters."}</p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setNotifyPicker(null)}>
            <div className="w-full max-w-lg rounded-2xl border border-sky-500/30 bg-[#0f0b1a] p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className="text-[11px] font-bold text-white/75">Bot:</span>
                ...
                <span className="ml-2 text-[11px] font-bold text-white/75">Mode:</span>
                {(["build", "improve"] as const).map((m) => (
                  <button key={m} onClick={() => setNotifyMode(m)} className={`rounded-full px-3 py-1.5 text-xs font-bold border capitalize ${notifyMode===m ? "bg-violet-600 text-white border-violet-600" : "bg-white/5 text-white/75 border-white/10 hover:text-white"}`}>{m}</button>
                ))}
              </div>
