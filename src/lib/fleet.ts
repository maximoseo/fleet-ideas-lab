...
export const FLEET_IDEAS: FleetIdea[] = [
  { id: "idea-1", title: "SERP Volatility War Room", slug: "serp-volatility-war-room", domain: "seo", whyNow: "August core update spiked volatility — clients need real-time triage.", widgets: ["Volatility Index", "Winners/Losers", "SERP Features", "Alert Feed"], effort: "M", priority: "P0", impact: "high", status: "new", dashboardUrl: "https://site-intel.maximo-seo.ai/serp", description: "Live SERP volatility dashboard with anomaly alerts and affected URL drill-down.", problem: "SERP volatility spikes leave clients blind — no real-time view of who won/lost or which SERP features shifted.", solution: "War Room: live Volatility Index + Winners/Losers table + SERP feature share + Alert feed with URL drill-down.", benefit: "Ops triage in minutes, not hours — client comms with evidence.", dataNeeded: "GSC Search Analytics + third-party SERP API (vault TBD) + Vercel cron hourly", feasibility: "Medium — needs SERP API quota + GSC OAuth, no invented metrics", nextStep: "Scaffold stub → wire GSC + SERP provider → ship volatility index first", prompt: "Build SERP Volatility War Room: real-time volatility index, winners/losers table, SERP feature share, alert feed. Data: GSC + third-party SERP API. Violet theme, 48dp targets." },
  { id: "idea-2", title: "Content Decay Radar", slug: "content-decay-radar", domain: "content", whyNow: "30% of blog traffic decays after 6mo — proactive refresh gap.", widgets: ["Decay Score", "Traffic Trend", "Refresh Queue", "Brief Generator"], effort: "M", priority: "P0", impact: "high", status: "scoped", dashboardUrl: "https://content-forge.maximo-seo.ai/decay", description: "Detects decaying content, scores decay risk, queues refresh briefs.", problem: "30% of blog traffic decays after 6 months — teams discover it too late and traffic never recovers.", solution: "Radar: Decay Score (traffic vs content age) + 90-day trend + Refresh Queue with one-click Brief generation.", benefit: "Recover 10-30% of decaying traffic via systematic refresh — compounding over quarters.", dataNeeded: "GSC + CMS publish dates (vault TBD) + content-age decay model", feasibility: "Medium — CMS dates are the blocker, GSC is ready", nextStep: "Scaffold → connect CMS dates → ship Decay Score + Queue first", prompt: "Build Content Decay Radar: decay score (traffic vs age), 90-day trend chart, refresh queue with one-click brief generation. Integrate GSC + CMS." },
  { id: "idea-3", title: "GBP Health Monitor", slug: "gbp-health-monitor", domain: "local", whyNow: "GBP suspensions up 40% YoY — early warning prevents revenue loss.", widgets: ["Suspension Risk", "Listing Completeness", "Photo Freshness", "Review Velocity"], effort: "S", priority: "P0", impact: "high", status: "new", dashboardUrl: "https://local-rank.maximo-seo.ai/health", description: "GBP listing health scoring with suspension risk and completeness checks.", problem: "GBP suspensions are up 40% YoY — one missed field or stale photo can suspend a revenue-driving listing.", solution: "Health Monitor: Suspension Risk + Listing Completeness checklist + Photo Freshness + Review Velocity chart, pulled daily.", benefit: "Prevents revenue loss from suspensions and boosts local pack eligibility.", dataNeeded: "GBP API daily pulls (vault TBD) + photo timestamps", feasibility: "Easy — GBP API is stable, no invented NAP", nextStep: "Scaffold → wire GBP API → ship Completeness + Risk first", prompt: "Build GBP Health Monitor: completeness checklist, suspension risk score, photo freshness, review velocity chart. Pull GBP API daily." },
  { id: "idea-4", title: "Anomaly Explain Engine", slug: "anomaly-explain-engine", domain: "analytics", whyNow: "Clients ignore alerts they don't understand — explainability gap.", widgets: ["Anomaly Timeline", "Root Cause", "Impact Estimate", "Suggested Action"], effort: "L", priority: "P1", impact: "high", status: "new", description: "Turns GA4/GSC anomalies into plain-English explanations with impact and fix.", problem: "Clients ignore alerts they do not understand — anomaly without explanation is noise.", solution: "Engine: Anomaly Timeline + LLM Root Cause + Impact Estimate (traffic/revenue) + one-click Suggested Action.", benefit: "Alerts become decisions — faster MTTR and fewer escalations.", dataNeeded: "GA4 + GSC anomaly stream + LLM for root-cause (vault TBD)", feasibility: "Large — LLM + anomaly detection are the hard parts", nextStep: "Scaffold → ship Timeline + Impact first, LLM root-cause phase 2", prompt: "Build Anomaly Explain Engine: timeline of anomalies, LLM root-cause, estimated traffic/revenue impact, one-click action. Use GA4 + GSC anomalies." },
  { id: "idea-5", title: "Outreach Inbox Commander", slug: "outreach-inbox-commander", domain: "outreach", whyNow: "Link builders lose 20% replies in Gmail noise — dedicated inbox needed.", widgets: ["Thread List", "Reply Score", "Follow-up Timer", "Template Inject"], effort: "M", priority: "P1", impact: "medium", status: "backlog", dashboardUrl: "https://outreach.maximo-seo.ai/inbox", description: "Gmail-synced outreach inbox with reply prediction and auto follow-ups.", problem: "Link builders lose ~20% of replies in Gmail noise — no dedicated outreach inbox with prioritization.", solution: "Inbox Commander: Thread List with Reply-Likelihood Score + Follow-up Timer + Template Inject.", benefit: "Recover lost replies and cut follow-up busywork in half.", dataNeeded: "Gmail API sync + reply prediction model (vault TBD)", feasibility: "Medium — Gmail sync is straightforward, scoring is P2", nextStep: "Scaffold → Gmail sync → ship Thread List + Timer first", prompt: "Build Outreach Inbox Commander: thread list with reply-likelihood score, follow-up timer, template insertion. Sync Gmail API." },
  { id: "idea-6", title: "Schema Studio", slug: "schema-studio", domain: "technical", whyNow: "Rich result eligibility drives CTR — schema errors are invisible revenue leaks.", widgets: ["Schema Validator", "Rich Result Preview", "Coverage Map", "Fix Diff"], effort: "S", priority: "P1", impact: "medium", status: "new", description: "Visual schema editor with validation, preview, and deploy diff.", problem: "Schema errors silently kill rich-result eligibility — invisible CTR loss.", solution: "Studio: JSON-LD Editor + Validator + Google Rich Result Preview + Fix Diff / Deploy gate.", benefit: "Recover rich-result eligibility and CTR with a visual safety net.", dataNeeded: "schema.org validator + Google Rich Results test (no invented markup)", feasibility: "Easy — editor + validator are commodity, preview is the win", nextStep: "Scaffold → ship Editor + Validator + Preview first", prompt: "Build Schema Studio: JSON-LD editor, validator, Google rich result preview, deploy diff. Validate against schema.org." },
  { id: "idea-7", title: "Design Token Pipeline", slug: "design-token-pipeline", domain: "design", whyNow: "Design Lab tokens are manual — automated WP injection unlocks scale.", widgets: ["Token Editor", "WP Sync Status", "Preview Frame", "Version History"], effort: "M", priority: "P1", impact: "medium", status: "scoped", dashboardUrl: "https://design-lab.maximo-seo.ai/tokens", description: "Design token editor with one-click WordPress injection pipeline.", problem: "Design Lab tokens are still manual — no automated pipeline to WordPress.", solution: "Pipeline: Token Editor + WP Sync Status + live Preview Frame + Version History with one-click Inject.", benefit: "Ship design-system changes to WordPress in one click instead of manual copy-paste.", dataNeeded: "Existing /api/wp/inject + /api/wp/theme-css (already live, vault TBD)", feasibility: "Medium — WP endpoints exist, Preview frame is the new piece", nextStep: "Scaffold → wire existing WP inject → ship Editor + Preview first", prompt: "Build Design Token Pipeline: token editor, WP connection status, live preview frame, version history. Reuse /api/wp/inject." },
  { id: "idea-8", title: "Content Brief Autopilot", slug: "content-brief-autopilot", domain: "content", whyNow: "Brief creation is #1 bottleneck — SERP-informed auto-briefs 10x throughput.", widgets: ["SERP Brief", "Outline Builder", "Entity Map", "Competitor Gap"], effort: "L", priority: "P0", impact: "high", status: "new", description: "Auto-generates SEO briefs from SERP analysis with entity and gap data.", problem: "Brief creation is the #1 content bottleneck — manual SERP reading does not scale.", solution: "Autopilot: input Keyword → SERP Brief + Outline Builder + Entity Map + Competitor Gap table.", benefit: "10x brief throughput with SERP-informed, entity-rich briefs.", dataNeeded: "SERP API + LLM for outline/entities (vault TBD, no invented volumes)", feasibility: "Large — SERP + LLM are the heavy lift", nextStep: "Scaffold → SERP fetch → ship SERP Brief + Outline first", prompt: "Build Content Brief Autopilot: inputs keyword, outputs SERP-informed brief with outline, entities, competitor gap table. LLM + SERP API." },
  { id: "idea-9", title: "Local Citation Pulse", slug: "local-citation-pulse", domain: "local", whyNow: "NAP inconsistencies still derail 1 in 5 local packs.", widgets: ["Citation Map", "NAP Diff", "Fix Queue", "Authority Score"], effort: "M", priority: "P2", impact: "medium", status: "backlog", description: "Citation consistency tracker across 40+ directories with fix queue.", problem: "NAP inconsistencies still derail 1 in 5 local packs — scattered across 40+ directories.", solution: "Pulse: Citation Map + NAP Diff highlighter + Fix Queue + Authority Score.", benefit: "Systematically fix local pack eligibility — measurable within weeks.", dataNeeded: "Whitespark / BrightLocal citations (vault TBD, no invented NAP)", feasibility: "Medium — citation API is the dependency", nextStep: "Scaffold → citation API → ship Map + Diff first", prompt: "Build Local Citation Pulse: map of citations, NAP diff highlighter, fix queue, authority score. Pull Whitespark/BrightLocal." },
  { id: "idea-10", title: "Fleet Cron Observatory", slug: "fleet-cron-observatory", domain: "automation", whyNow: "Fleet has 50+ crons — silent failures cost hours weekly.", widgets: ["Cron Timeline", "Failure Heatmap", "Run Logs", "Retry Control"], effort: "S", priority: "P0", impact: "high", status: "new", description: "All fleet crons in one timeline with failure heatmap and retry controls.", problem: "Fleet runs 50+ crons — silent failures cost hours every week with no single view.", solution: "Observatory: Cron Timeline + Failure Heatmap by hour + Run Logs + Retry Control (n8n + Vercel logs aggregated).", benefit: "Zero silent failures — weekly ops hours recovered.", dataNeeded: "n8n + Vercel cron logs aggregation (vault TBD)", feasibility: "Easy — log aggregation is straightforward, retry is P2", nextStep: "Scaffold → aggregate logs → ship Timeline + Heatmap first", prompt: "Build Fleet Cron Observatory: timeline of all cron runs, failure heatmap by hour, log viewer, retry button. Aggregate n8n + Vercel cron logs." },
  { id: "idea-11", title: "Link Velocity Tracker", slug: "link-velocity-tracker", domain: "outreach", whyNow: "Link velocity is leading indicator — competitors accelerating unseen.", widgets: ["Velocity Chart", "New/Lost Links", "Anchor Mix", "Risk Flag"], effort: "M", priority: "P2", impact: "low", status: "new", description: "Link acquisition velocity with new/lost, anchor mix, and risk flags.", problem: "Competitor link velocity is a leading indicator — invisible until it already beat you.", solution: "Tracker: 90-day Velocity Chart + New/Lost Links table + Anchor Mix + Toxic Risk Flag.", benefit: "Spot competitor acceleration early and calibrate outreach pace.", dataNeeded: "Ahrefs / Majestic link history (vault TBD, no invented links)", feasibility: "Medium — third-party link API is the gate", nextStep: "Scaffold → link API → ship Velocity Chart + New/Lost first", prompt: "Build Link Velocity Tracker: 90-day velocity chart, new/lost table, anchor distribution, toxic risk flag. Use Ahrefs/Majestic." },
  { id: "idea-12", title: "CWV Budget Guard", slug: "cwv-budget-guard", domain: "technical", whyNow: "CWV regressions ship silently — budget guard blocks deploys on breach.", widgets: ["CWV Gauges", "Budget Bar", "Regression Diff", "Deploy Gate"], effort: "S", priority: "P1", impact: "high", status: "new", description: "Core Web Vitals budget enforcement with deploy gate and regression diffs.", problem: "CWV regressions ship silently and erode rankings and UX before anyone notices.", solution: "Guard: LCP/CLS/INP Gauges + Budget Bar + Regression Diff vs last deploy + Deploy Gate that blocks ship on breach.", benefit: "Block regressions before they reach users — performance as a gate, not a hope.", dataNeeded: "Lighthouse CI + deploy diff (no invented scores)", feasibility: "Easy — Lighthouse CI is well-trodden, gate is the product", nextStep: "Scaffold → Lighthouse CI → ship Gauges + Gate first", prompt: "Build CWV Budget Guard: LCP/CLS/INP gauges, budget bar, regression diff vs last deploy, gate that blocks ship on breach. Lighthouse CI." },
];
